...
class GameEvaluator:
    ...
    def judge(self, table):
        winners = self.__find_winners_from(
            table.get_community_card(), table.seats.players
        )
        hand_info = self.__gen_hand_info_if_needed(
            table.seats.players, table.get_community_card()
        )
        prize_map = self.__calc_prize_distribution(
            table.get_community_card(), table.seats.players
        )
    ...
    def __find_winners_from(self, community_card, players):
        ...
        score_with_players = [
            (score, player) for score, player in zip(scores, active_players)
        ]
    ...
    def __gen_hand_info_if_needed(self, players, community):
        ...
        return (
            ...
            else [gen_hand_info(player) for player in active_players]
        )
    ...
    def __get_side_pots(self, players):
        pay_amounts = [
            payinfo.amount for payinfo in self.__fetch_allin_payinfo(players)
        ]
    ...
    def __create_sidepot(self, players, smaller_side_pots, allin_amount):
        return {
            "amount": self.__calc_sidepot_size(
                players, smaller_side_pots, allin_amount
            ),
    ...
    def __calc_sidepot_size(self, players, smaller_side_pots, allin_amount):
        add_chip_for_pot = lambda pot, player: pot + min(
    ...
    def __select_eligibles(self, players, allin_amount):
        return [
            player for player in players if self.__is_eligible(player, allin_amount)
        ]
