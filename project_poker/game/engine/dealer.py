...
class Dealer:
    ...
    def __is_game_finished(self, table):
        return (
            len([player for player in table.seats.players if player.is_active()]) == 1
        )
    ...
    def __message_check(self, msgs, street):
        ...
        invalid &= (
            street != Const.Street.FINISHED
            or msg["message"]["message_type"] == "round_result"
        )
        if invalid:
    ...
    def __steal_money_from_poor_player(self, table, ante, sb_amount):
        ...
        search_targets = search_targets[
            table.dealer_btn + 1 : table.dealer_btn + 1 + len(players)
        ]
        ...
        search_targets = search_targets[
            sb_relative_pos + 1 : sb_relative_pos + len(players)
        ]
    ...
    def __find_first_elligible_player(self, players, need_amount, default=None):
        if default:
            return next(
                (player for player in players if player.stack >= need_amount), default
            )
...
class MessageSummarizer(object):
    ...
    def summairze_blind_level_update(
        self, round_count, old_ante, new_ante, old_sb_amount, new_sb_amount
    ):
